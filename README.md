...
- по умолчанию показаны **свободные** станции; статус задаётся в адресе, поэтому есть прямые ссылки:
  `/merchants/<id>` — свободные, `/merchants/<id>?status=all` — все, `/merchants/<id>?status=busy` — занятые;
- фильтры по видеокарте и **игре** («на каких станциях есть Cyberpunk»);
...
- **каталог «Игры»** `/merchants/<id>/games` — аналог drova.io/games, но только игры наших станций: популярные первыми
  (`/accounting/statistics/most_popular_games`), «на N ст. · свободно M», фильтры «Игра / Лицензия / Учётная запись»
  и «Только со свободными станциями». Клик по игре — окно «Где играть в …»: «Любая свободная станция» (автоподбор
  свободной с наименьшим номером) или выбор из списка; выбор ведёт на нашу страницу станции с этой игрой
  (`/merchants/<id>/stations/<stationId>?game=<productId>`);
...
| `GET /accounting/statistics/most_popular_games` | порядок игр по популярности на drova.io |
